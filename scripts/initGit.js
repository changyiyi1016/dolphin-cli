...
const initGit = async (projectName) => {
    ...
    await execSync('git push --set-upstream ssh://git@github.com:changyiyi1016/$(git rev-parse --show-toplevel | xargs basename).git $(git rev-parse --abbrev-ref HEAD)', { cwd: join(process.cwd(), projectName)})
}
